"""4개 소스 모듈을 import 해 레지스트리에 등록하는 부작용을 낸다.

이 패키지를 import 하면 openalex/crossref/semantic_scholar/europepmc 4개
모듈이 각자 @register 되어 paper_radar.registry.SOURCES 에 들어간다. 소스
자체를 쓰려면 각 서브모듈을 직접 import 해서 그 안의 search/trend/fetch
함수를 호출한다 — 이 파일은 등록 트리거일 뿐이다.
...
설계 사유 — 엔진/Lane 이 없는 이유
    ...
    소스도 4개뿐이라 순차 드라이버 — search/trend/fetch 가 Transport 를 직접
...
__all__ = ["crossref", "europepmc", "openalex", "semantic_scholar"]
