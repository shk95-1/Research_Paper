...
class TransientError(TransportError):
    """RETRY_STATUS 에 포함된 5xx 의 재시도 소진, RETRY_STATUS 밖의 5xx(예: 501/505,
    재시도 대상 목록에 없다는 이유만으로 재시도하지는 않지만 서버측 오류라는 사실은
    같으므로 즉시 이 타입으로 던진다), 연결 실패, 타임아웃. 호출자 조치: 이 요청만
    실패로 기록하고 나머지 수집은 계속한다 — 다음 실행에서 재시도될 수 있다.
    """
...
class PermanentError(TransportError):
    """404/402/409 를 제외한 그 외 4xx, 그리고 5xx 가 아닌 그 밖의 예상치 못한 상태
    (예: 예상 밖 3xx). 요청 자체가 잘못됐다는 뜻.
