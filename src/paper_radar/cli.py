...
"그 주장 뒤에 대조시험이 있다"의 차이를 만든다. collect 는 evidence collect
와 같은 결로 RunLog 자기기록 + iter_studies() 점진 소비(부분 결과 보존) +
upsert_records() 를 하되, 보강(enrich) 단계가 없어 훨씬 단순하다 — 그래서
별도 evidence.pipeline 급 모듈을 새로 두지 않고 _collect_trials() 로 이
파일 안에 둔다. status 판정("ok"/"partial")은 그 함수 안에서만 계산하고
...
from paper_radar.sources import clinicaltrials, openalex
...
def parse_args(argv):
    ...
    trials_collect = trials_sub.add_parser("collect", help="임상시험 수집 (네트워크)")
    trials_collect.add_argument("--query", required=True, help="영문 검색어")
    trials_collect.add_argument("--limit", type=int, default=DEFAULT_LIMIT)

    trials_list = trials_sub.add_parser("list", help="저장된 임상시험 검색 (로컬 전용)")
    trials_list.add_argument("--keyword", required=True)
    trials_list.add_argument("--limit", type=int, default=20)

    for sub in (trials_collect, trials_list):
        sub.add_argument("--db", default=DEFAULT_DB, help=argparse.SUPPRESS)
...
class TrialsReport:
    """_collect_trials() 실행 결과 요약. CLI 는 이 값으로 exit code 를 정한다.

    evidence.pipeline.CollectReport 와 같은 결의 "단일 출처" 계약이다 —
    status("ok"/"partial")는 이 dataclass 를 만드는 _collect_trials() 안에서만
    계산하고, CLI(_run_trials_collect)는 그 값을 그대로 옮길 뿐 stopped_reason
    을 다시 훑어 재계산하지 않는다. "failed"는 _collect_trials() 가 처리하지
    못한 예외로 끝나는 경우라 이 필드로는 관측되지 않는다(그 경로에서는
    TrialsReport 자체가 만들어지지 않고 예외가 호출자까지 전파된다).
    """
...
def _collect_trials(conn, transport, query, limit, *, on_progress=None) -> TrialsReport:
    """검색 -> 저장, 그리고 실행 자체를 RunLog 에 기록한다.

    evidence.pipeline.collect() 와 같은 골격(RunLog.start -> 점진 소비 ->
    upsert -> record_source -> finish)이지만 보강(enrich) 단계가 없어 훨씬
    단순하다 — clinicaltrials.iter_studies() 를 제너레이터인 채로 직접
    순회해서(list() 로 감싸지 않고) 중간 페이지에서 예외가 나도 이미 받은
    레코드를 보존한다(openalex.iter_search()/evidence.pipeline.collect() 와
    같은 이유 — clinicaltrials.py 모듈 docstring 참고).
    ...
    """
    run = RunLog(conn)
    run_id = run.start("trials collect", {"query": query, "limit": limit})
...
def _run_trials_collect(args):
    ...
    conn = repository.connect(args.db)
    try:
        report = _collect_trials(conn, Transport(), args.query, args.limit, on_progress=progress)
    finally:
        conn.close()

    print(f"\n저장 완료: {len(report.records)}건 -> {args.db}")
    # "ok"/"partial" 판정은 _collect_trials() 한 곳에서만 계산한다(단일 출처) —
    # CLI 는 그 결과(report.status)를 그대로 exit code 로 옮길 뿐, 여기서
    # stopped_reason 을 다시 훑어 재계산하지 않는다.
    return 0 if report.status == "ok" else 1
